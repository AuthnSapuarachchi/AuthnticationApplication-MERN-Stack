...
const Login = () => {
  ...
  return (
    <div className='flex items-center justify-center min-h-screen px-6 sm:px-0 bg-gradient-to-r from-blue-200 to-purple-400 relative'>
      ...
      <div className='bg-slate-900 p-10 rounded-lg shadow-lg top-5 w-full sm:w-96 text-indigo-300 cursor-pointer text-sm'>
        <h2>{state === 'Sign Up' ? 'Create Account' : 'Login'}</h2>
        <p>{state === 'Sign Up' ? 'Create your account' : 'Login to your account'}</p>
        <form>
          <div className=' mb-4 flex items-center gap-4 w-full px-5 py-2.5 bg-[#333A5C] rounded-md shadow-md'>
            <img src={assets.person_icon} alt="" />
            <input type="text" placeholder='Full Name' className='bg-transparent outline-none border-radius' required/>
