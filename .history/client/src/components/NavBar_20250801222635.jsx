...
const NavBar = () => {
    ...
    const {userData, setUserData, setIsLoggedIn, backendUrl} = useContext(AppContext);    const logout = async () => {
      try {
          ...
          <div className='w-8 h-8 flex items-center justify-center rounded-full bg-black text-white font-medium relative group'>
            ...
            <div className='absolute hidden group-hover:block top-0 right-0 z-10 text-black rounded pt-10'>
              <ul className='list-none m-0 p-2 bg-gray-100 text-sm'>
                {!userData.isAccountVerified &&  <li onClick={() => navigate('/email-verify')} className='py-1 px-2 hover:bg-gray-200 cursor-pointer'>Verify email</li> }
