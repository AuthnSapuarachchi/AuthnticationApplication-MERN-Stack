...
const Login = () => {
  ...
  const [state, setState] = useState('Login'); // Default to Login instead of Sign Up
  const [name, setName] = useState('');
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');

  // Check URL parameters or state to determine if we should show signup
  useEffect(() => {
  ...
  return (
    <div className='flex items-center justify-center min-h-screen px-6 sm:px-0 bg-gradient-to-r from-blue-200 to-purple-400 relative'>
      <div className='text-xl sm:text-2xl font-bold text-gray-800 cursor-pointer hover:text-blue-600 transition-colors' onClick={() => navigate('/')}>
        AuthBuilder
      </div>
      <div className='bg-slate-900 p-10 rounded-lg shadow-lg top-5 w-full sm:w-96 text-indigo-300 text-sm'>
        ...
        { state === 'Sign Up' ? (
          ...
          <span className='text-blue-400 cursor-pointer underline' onClick={() => navigate('/signup')}>
            Sign up
          </span>
          </p> )}     
      </div>
